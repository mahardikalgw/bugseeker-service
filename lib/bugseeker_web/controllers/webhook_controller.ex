defmodule BugseekerWeb.WebhookController do
  @moduledoc """
  Receives GitHub webhooks, verifies the HMAC-SHA256 signature, filters
  events, and hands the work to `Bugseeker.CoordinatorSup` (or the
  `/bugseeker` command handler) so the response can be sent immediately.
  ...
  alias Bugseeker.{Commands, Jobs.FetchDiffJob}
  ...
  defp verify_signature(conn, raw) do
    secret = get_in(Application.get_env(:bugseeker, :github), [:webhook_secret])
  ...
  defp dispatch_issue_comment(conn, payload) do
    with %{"action" => "created"} <- payload,
         ...
         true <- is_binary(body) and String.starts_with?(body, "/bugseeker") do
