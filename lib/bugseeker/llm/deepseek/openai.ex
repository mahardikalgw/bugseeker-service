defmodule Bugseeker.Llm.DeepSeek.OpenAI do
  @moduledoc """
  DeepSeek chat completions client backed by Req, using the OpenAI-compatible
  `/chat/completions` endpoint with JSON mode.
  """

  @behaviour Bugseeker.Llm.DeepSeek

  require Logger

  @impl true
  def chat(prompt) do
    api = Application.get_env(:bugseeker, :llm_api, %{})
    llm = Application.get_env(:bugseeker, :llm, %{})
