defmodule Bugseeker.PerRepo do
  @moduledoc """
  ...
  Initialized from `config :bugseeker, :repos` at boot; mutated at
  runtime by `/bugseeker` commands. Runtime-only changes are lost on
  restart — persist them in `config :bugseeker, :repos`.
  """
  ...
  def start_link(_opts) do
    Agent.start_link(fn -> Application.get_env(:bugseeker, :repos, %{}) end, name: __MODULE__)
  end
  ...
  def min_inline_severity(repo) do
    get(repo)[:min_inline_severity] ||
      Application.get_env(:bugseeker, :min_inline_severity, "HIGH")
  end
