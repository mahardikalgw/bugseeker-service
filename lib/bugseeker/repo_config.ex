defmodule Bugseeker.RepoConfig do
  @moduledoc """
  Reads the reviewed repository's own `.bugseeker.yml` (or
  `.bugseeker.yaml`) and turns it into review settings.
  ...
  Precedence: `.bugseeker.yml` in the repo > `Bugseeker.PerRepo`
  ...
  alias Bugseeker.{Agents.Cache, Clients}

  @paths [".bugseeker.yml", ".bugseeker.yaml"]
  @severities ~w(CRITICAL HIGH MEDIUM LOW INFO)

  @type t :: %{optional(:agents) => [String.t()], optional(:min_inline_severity) => String.t()}

  @doc """
  Fetches and parses the repo's `.bugseeker.yml` at `ref`.
  ...
  @doc """
  Parses `.bugseeker.yml` content into a config map. Returns `nil` for
