defmodule BugseekerWeb.WebhookControllerTest do
  use BugseekerWeb.ConnCase, async: false

  import Mox

  alias Bugseeker.Github.Client.Mock, as: GithubMock

  alias BugseekerWeb.WebhookTestHelper
  ...
  describe "signature verification" do
    test "accepts a valid signature and enqueues a FetchDiffJob" do
      ...
      assert length(enqueued(Bugseeker.Jobs.FetchDiffJob)) == 1
    end
    ...
    test "rejects a tampered body with 401" do
      ...
      assert enqueued(Bugseeker.Jobs.FetchDiffJob) == []
    end
  ...
  describe "event filtering" do
    test "opened and synchronize actions enqueue a FetchDiffJob" do
      ...
      assert length(enqueued(Bugseeker.Jobs.FetchDiffJob)) == 2
    end
  ...
  describe "issue_comment commands" do
    test "/bugseeker status replies via the GitHub API" do
      test_pid = self()
