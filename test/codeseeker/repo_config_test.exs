defmodule Codeseeker.RepoConfigTest do
  ...
  describe "fetch/2" do
    test "reads .codeseeker.yml from the repo at the given ref" do
      expect(GithubMock, :get_raw_contents, fn _repo, ".codeseeker.yml", "abc123" ->
    ...
    test "falls back to .codeseeker.yaml" do
      expect(GithubMock, :get_raw_contents, fn _repo, ".codeseeker.yml", _ref ->
        {:error, :not_found}
      end)

      expect(GithubMock, :get_raw_contents, fn _repo, ".codeseeker.yaml", _ref ->
        {:ok, "agents:\n  - nestjs_security\n"}
      end)
