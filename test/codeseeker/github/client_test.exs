defmodule Codeseeker.Github.ClientTest do
  use ExUnit.Case, async: true

  alias Codeseeker.Github.{Client.Github, Error}

  setup do
    bypass = Bypass.open()
    installation_id = :erlang.unique_integer([:positive])
    Application.put_env(:codeseeker, :github_api_url, "http://localhost:#{bypass.port}")
    token_tid = :ets.new(:token_calls, [:public])

    on_exit(fn -> Application.delete_env(:codeseeker, :github_api_url) end)
